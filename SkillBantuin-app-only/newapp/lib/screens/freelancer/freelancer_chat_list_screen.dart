...
class _FreelancerChatListScreenState extends State<FreelancerChatListScreen> {
  ...
  void _navToRoom(ChatRoom room) {
    ...
    Navigator.push(context, PageRouteBuilder(
      pageBuilder: (_, __, ___) => ChatRoomScreen(
        room: room, currentRole: UserRole.freelancer),
  ...
  // ── Open Demo Chat — untuk freelancer, tampilkan info akun demo ──
  Future<void> _openDemoChat() async {
    HapticFeedback.lightImpact();
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      builder: (_) => _DemoInfoSheet(),
    );
  }
    ...
    child: Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Chat Demo dengan Client',
            style: TextStyle(color: Colors.white,
              fontWeight: FontWeight.w800, fontSize: 13.5)),
          const Text('Coba kirim pesan — bisa diterima akun client@demo.com',
...
// ── Demo Info Sheet (untuk freelancer) ───────────────────────────────────────
class _DemoInfoSheet extends StatelessWidget {
  @override
  Widget build(BuildContext context) => Container(
    ...
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      const Text('🧪 Testing Chat Demo', style: TextStyle(
        fontSize: 17, fontWeight: FontWeight.w900, color: FPal.ink)),
      const SizedBox(height: 12),
      const Text(
        'Untuk menguji fitur chat antar role, gunakan akun demo berikut. '
        'Login di device/tab berbeda, lalu kirim pesan:',
        textAlign: TextAlign.center,
        style: TextStyle(fontSize: 13, color: FPal.inkMuted, height: 1.4)),
      const SizedBox(height: 16),
      _CredCard('👤 Client', 'client@demo.com', 'demo12345', FPal.primary),
      const SizedBox(height: 10),
      _CredCard('💼 Freelancer', 'freelancer@demo.com', 'demo12345',
      ...
      Container(
        ...
        child: const Row(children: [
          Icon(Icons.lightbulb_outline_rounded, color: Color(0xFFD97706), size: 16),
          SizedBox(width: 8),
          Expanded(child: Text(
            'Setelah login sebagai client, tap tombol "Chat Demo" di menu Chat untuk mulai percakapan.',
...
class _CredCard extends StatelessWidget {
  ...
  final Color color;
  const _CredCard(this.role, this.email, this.pass, this.color);
  @override
  Widget build(BuildContext context) => Container(
    padding: const EdgeInsets.all(14),
    decoration: BoxDecoration(
    ...
    child: Row(children: [
      Text(role, style: TextStyle(fontWeight: FontWeight.w800, color: color, fontSize: 13)),
      const Spacer(),
      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
        Text(email, style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600, color: color)),
        Text('Password: $pass', style: TextStyle(fontSize: 11.5, color: color.withOpacity(0.7))),
      ]),
    ]));
}
